...
def initialize_state_deployed(callback_context: CallbackContext):
    ...
    if user_content and user_content.parts:
        try:
            ...
            # Handle file-based input - prefer direct source code if available
            if 'source_code' in initial_data and initial_data['source_code']:
                # Direct source code input (preferred)
                source_code = initial_data.get('source_code')
                language = initial_data.get('language', 'python')
                print(f"Using direct source code input ({len(source_code)} characters)")
                print(f"Detected language: {language}")
            elif 'file_url' in initial_data:
                # Download file from GCS bucket
                print(f"Attempting to download from GCS: {initial_data['file_url']}")
            ...
            print(f"State initialized - Language: {language}, Source code length: {len(source_code)}")
...
def download_file_from_gcs(file_url: str) -> str:
    ...
    try:
        print(f"Attempting to download from GCS: {file_url}")
        ...
        if file_url.startswith('gs://'):
            ...
            if not blob.exists():
                ...
                raise Exception(f"File not found: {blob_name}")
        ...
        # Return sample C code for testing
        sample_c_code = """
#include <stdio.h>
#include <stdlib.h>

// Sample C functions for testing
int add(int a, int b) {
    return a + b;
}
...
"""
        print("Using sample C code for testing...")
        return sample_c_code
...
def create_workflow_agents_deployed(language: str = "python"):
    ...
    def debug_callback(agent_name):
        def callback(tool: BaseTool, args: dict, tool_context: ToolContext, tool_response: dict):
            ...
            if hasattr(tool_context.state, 'keys'):
                ...
                if 'test_scenarios' in tool_context.state:
                    scenarios = tool_context.state['test_scenarios']
                    print(f"Test scenarios: {len(scenarios)} items")
                    if scenarios and len(scenarios) > 0:
                        print(f"First scenario: {scenarios[0]}")
                if 'test_code' in tool_context.state:
                    print(f"Test code length: {len(tool_context.state['test_code'])}")
                if 'source_code' in tool_context.state:
                    print(f"Source code length: {len(tool_context.state['source_code'])}")
                if 'language' in tool_context.state:
                    print(f"Language: {tool_context.state['language']}")
            else:
                print(f"State content: {tool_context.state}")
        return callback
    
    test_case_designer.after_tool_callback = debug_callback("TestCaseDesigner")
    test_implementer.after_tool_callback = debug_callback("TestImplementer")
